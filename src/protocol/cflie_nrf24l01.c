...
static const char * const cflie_opts[] = {
    ...
    _tr_noop("Max Ang Level"), "0", "1000", STEP_SIZE, NULL,
    _tr_noop("Max AngV Rate"), "0", "1000", STEP_SIZE, NULL,
    NULL
};

enum {
    PROTOOPTS_TELEMETRY = 0,
    PROTOOPTS_MAX_ANG_LEVEL,
    PROTOOPTS_MAX_ANGV_RATE,
    LAST_PROTO_OPT,
};
...
// Flight modes -- maps to RPType in the cflie firmware
#define MODE_RATE   0
#define MODE_LEVEL  1
...
static u8 packet_ack()
{
    ...
    case BV(NRF24L01_07_TX_DS):
        rx_payload_len = NRF24L01_ReadPayloadLength();
...
static void send_cmd_packet()
{
    ...
    // Switch on indicates level mode
    if (Channels[5] >= 0) {
        cpkt.mode = MODE_LEVEL;
    } else {
        cpkt.mode = MODE_RATE;
    }
    ...
    if(cpkt.mode == MODE_LEVEL) {
        // Roll, aka aileron, float +- MAX_ROLL in degrees
        // float roll  = -(float) Channels[0]*MAX_ROLL/10000;
        f_roll = -Channels[0] * FRAC_SCALE / (10000 / Model.proto_opts[PROTOOPTS_MAX_ANG_LEVEL]);

        // Pitch, aka elevator, float +- MAX_PITCH degrees
        // float pitch = -(float) Channels[1]*MAX_PITCH/10000;
        f_pitch = -Channels[1] * FRAC_SCALE / (10000 / Model.proto_opts[PROTOOPTS_MAX_ANG_LEVEL]);
    } else {
        // Roll, aka aileron, float +- MAX_ROLL in degrees
        // float roll  = -(float) Channels[0]*MAX_ROLL/10000;
        f_roll = -Channels[0] * FRAC_SCALE / (10000 / Model.proto_opts[PROTOOPTS_MAX_ANGV_RATE]);

        // Pitch, aka elevator, float +- MAX_PITCH degrees
        // float pitch = -(float) Channels[1]*MAX_PITCH/10000;
        f_pitch = -Channels[1] * FRAC_SCALE / (10000 / Model.proto_opts[PROTOOPTS_MAX_ANGV_RATE]);
    }
...
static u8 telemetry_setup_state_machine()
{
    ...
    static u8 toc_size = 0;
    static u8 next_toc_variable = 0;
    static u8 vbat_var_id = 0;
    static u8 extvbat_var_id = 0;
    ...
    case CFLIE_TELEM_SETUP_STATE_ACK_CMD_GET_ITEM:
        if (packet_ack() == PKT_ACKED) {
            if (rx_payload_len >= 3
                ...
                // group and name (back to back null terminated strings
                // starting with the fifth byte) and see if it matches
                // any of the variables we need for logging
                //
                // Currently enabled for logging:
                //  - vbatMV (LOG_UINT16)
                //  - extVbatMV (LOG_UINT16)

                // Currently all variables being logged are UINT16, so
                // there is no need to perform the string check on non-UINT16
                // TOC elements
                if (rx_packet[3] == LOG_UINT16) {

                    char pm_group_name[] = "pm";
                    char vbat_var_name[] = "vbatMV";
                    char extvbat_var_name[] = "extVbatMV";

                    // For some reason, comparative functions
                    // (strcmp and memcmp) aren't working right.
                    // Compare each byte for now. This should be
                    // corrected someday.
                    if (rx_packet[4] == pm_group_name[0] &&
                        rx_packet[5] == pm_group_name[1] &&
                        rx_packet[6] == pm_group_name[2] &&
                        rx_packet[7] == vbat_var_name[0] &&
                        rx_packet[8] == vbat_var_name[1] &&
                        rx_packet[9] == vbat_var_name[2] &&
                        rx_packet[10] == vbat_var_name[3] &&
                        rx_packet[11] == vbat_var_name[4] &&
                        rx_packet[12] == vbat_var_name[5] &&
                        rx_packet[13] == vbat_var_name[6]) {
                    ...
                    if (rx_packet[4] == pm_group_name[0] &&
                        rx_packet[5] == pm_group_name[1] &&
                        rx_packet[6] == pm_group_name[2] &&
                        rx_packet[7] == extvbat_var_name[0] &&
                        rx_packet[8] == extvbat_var_name[1] &&
                        rx_packet[9] == extvbat_var_name[2] &&
                        rx_packet[10] == extvbat_var_name[3] &&
                        rx_packet[11] == extvbat_var_name[4] &&
                        rx_packet[12] == extvbat_var_name[5] &&
                        rx_packet[13] == extvbat_var_name[6] &&
                        rx_packet[14] == extvbat_var_name[7] &&
                        rx_packet[15] == extvbat_var_name[8] &&
                        rx_packet[16] == extvbat_var_name[9]) {
    ...
    case CFLIE_TELEM_SETUP_STATE_SEND_CONTROL_CREATE_BLOCK:
        ...
        tx_packet[2] = CFLIE_TELEM_LOG_BLOCK_ID; // Log block ID 1 -- probably shouldn't hard code this..
        tx_packet[3] = LOG_UINT16; // Variable type
        tx_packet[4] = vbat_var_id; // ID of the VBAT variable
        tx_packet[5] = LOG_UINT16; // Variable type
    ...
    case CFLIE_TELEM_SETUP_STATE_ACK_CONTROL_CREATE_BLOCK:
        if (packet_ack() == PKT_ACKED) {
            if (rx_payload_len >= 2
                    && rx_packet[0]
                            == crtp_create_header(CRTP_PORT_LOG,
                                    CRTP_LOG_CHAN_SETTINGS)
...
const void *CFlie_Cmds(enum ProtoCmds cmd)
{
    ...
    switch(cmd) {
        ...
        case PROTOCMD_NUMCHAN: return (void *) 6L; // A, E, T, R, + or x mode, acro/level mode
        case PROTOCMD_DEFAULT_NUMCHAN: return (void *)6L;
        case PROTOCMD_CURRENT_ID: return Model.fixed_id ? (void *)((unsigned long)Model.fixed_id) : 0;
        case PROTOCMD_GETOPTIONS:
            // If not yet set, pick some defaults
            if (Model.proto_opts[PROTOOPTS_MAX_ANG_LEVEL] == 0
                    && Model.proto_opts[PROTOOPTS_MAX_ANGV_RATE] == 0) {
                Model.proto_opts[PROTOOPTS_MAX_ANG_LEVEL] = 50;
                Model.proto_opts[PROTOOPTS_MAX_ANGV_RATE] = 1080;
            }
            return cflie_opts;
